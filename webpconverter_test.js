...
var enwebp=require('./cwebp-test.js');//get cwebp module(converts other image format to webp)
var dewebp=require('./dwebp-test.js');//get dwebp module(converts webp format to other image)
var gifwebp=require('./gwebp-test.js');//get gif2webp module(convert git image to webp)
var webpmux=require('./webpmux-test.js');//get webpmux module(convert non animated webp images to animated webp)
var debug=require('debug')('webp-converter-test');//Debug module, call "DEBUG=webp-converter node yourAppHere.js" to debug
...
//enwebp() return which platform webp library should be used for conversion
 exec(enwebp(),query.split(/\s+/), function (error, stdout, stderr) //execute command
    ...
    if(error){
      debug(error);
 			callback(error, 101);//return error code
 		}else {
  			callback(null, 100);//return success code
 	      }
 });
};
